...
export async function analyzeFrame(imageBase64: string, currentInstruction: string, successCriteria: string, extractionSchema?: import("./types").ExtractionField[], expectedDomain?: string, previousFrameDescription?: string) {
  return getVisionProvider().analyzeFrame(imageBase64, currentInstruction, successCriteria, extractionSchema, expectedDomain, previousFrameDescription);
}
